...
def test_cap_boundary_in_sane_range():
    ...
    Nz = np.array([100, 0, 0])
    Cz = np.array([0, 0, 100])
    ear_mid = np.array([0, 0, 0])
    cap_z, *_ = detect_cap_boundary(verts, Nz, Cz, ear_mid, mid_y=0.0)
...
def test_face_mask_region_semantics():
    ...
    mask, _ = face_mask_from_landmarks(verts, Nz, Iz, Cz, Lpa, Rpa, cap_z=20.0)
...
def test_face_mask_ear_sphere():
    ...
    mask, _ = face_mask_from_landmarks(
        verts, Nz, Iz, Cz, Lpa, Rpa, cap_z=10.0, ear_delete_radius=40.0
    )
    assert mask[0]
...
def test_full_anonymization_pipeline(
    ...
    cap_z, *_ = detect_cap_boundary(verts, Nz, Cz, ear_mid, mid_y)
    Iz = lm_n.sel(label="Iz").values
    mask, _ = face_mask_from_landmarks(verts, Nz, Iz, Cz, Lpa, Rpa, cap_z=cap_z)
